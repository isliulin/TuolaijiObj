...
{		
	SYSTICK_InitType systick_init_structure;
	systick_init_structure.SYSTICK_PeriodMs = 100;
	systick_init_structure.SYSTICK_PeriodUs = 0;
	systick_init_structure.SYSTICK_Isr = SysTick_Handler;
	LPLD_SYSTICK_Init(systick_init_structure);
}

/*里程计算时间*/
UINT32 TotalDistanceTimes  = 0;
uint16 pulseNum;//[10],pulseCnt=0,pulseFlag=0;
void SysTick_Handler(void)//100ms
{
  //uint16 speed;
     //   uint32_t a = SysTick->CTRL;
	//GPIOD_PTOR = GPIO3; 

       GPIOE_PTOR = GPIO7;
        
      TotalDistanceTimes++;    
//#if  defined(ZB223BYY0A) 
     // if(pulseFlag==0&&pulseCnt==1)
     // {
     // }
      //pulseCnt++
     // if(pulseCnt>9) pulseCnt=0; //采样10次
      //pulseNum[i] = LPLD_LPTMR_GetPulseAcc();  
 //     pulseNum = LPLD_LPTMR_GetPulseAcc();
 //     speed =  calcSpeed(pulseNum*10);//100ms  = 10v
