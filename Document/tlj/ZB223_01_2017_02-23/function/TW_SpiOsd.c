...
uchar shadow_r4e0,shadow_r4e1;
BYTE McuSpiClkToPclk(BYTE divider)  //27/1.5
{
	
	WriteTW88Page(PAGE4_CLOCK);
	shadow_r4e0=ReadTW88(0xE0);
	
	shadow_r4e1 = ReadTW88(0xE1);
	WriteTW88(0xE0, shadow_r4e0 & 0xFE);
	WriteTW88(0xE1, 0x20 | divider);
...
void McuSpiClkRestore(void)
{
	if(shadow_r4e0 & 0x01) {
		WriteTW88Page(PAGE4_CLOCK);
		WriteTW88(0xE0, shadow_r4e0);
		WriteTW88(0xE1, shadow_r4e1);
	}
}
...
{
	unsigned char ID_Code = 0;
        unsigned int cnt = 0;
	FeedWatchDog();
	Clear_External_WatchDog();
	WriteTW88Page( 0 );
	while(ID_Code!=TW88XX_PRODUCT_ID_CODE)
	{   
		 WriteTW88Page( 0 );
		ID_Code = ReadTW88(0);
		cnt++;
...
BYTE SPI_QUADInit(void)
{
	...
	if ( (temp == 0xC2) || ( temp == 0x1c)|| (temp == 0x9d)) {		// for MX	&& EON  && STM
		if(temp ==0x1C)
...
void McuSpiClkSet(BYTE PCLK_SEL,BYTE SPI_CK_SEL,BYTE SPI_CK_DIV) 
{
	...
	else			WriteTW88(0xE0, ReadTW88(0xE0) & ~0x01);         //0
	WriteTW88(0xE1, (SPI_CK_SEL << 4) | SPI_CK_DIV);           //0x21
}
//////////////////////////////////////////////////////////////////////
...
const FONT_SPI_INFO_t WORD_16_26_NEW 	= {0xFE0000, 0x2080, 16, 26, 0x7f, 0x07f, 0x07f, 0x07f};
void InitSystem(BYTE special)
{
	...
	McuSpiClkSet(PCLK_SEL_PCLK,SPI_CK_SEL_PCLK,SPI_CK_DIV_1P5);  
...
void SOsdHwBuffClean(void)
{
    ...
    SOsdHwBuff_rle_bpp = 0;
    SOsdHwBuff_rle_win=0;
    SOsdHwBuff_rle_count = 0;
    for(i=0; i<=8; i++)
    {
        //clear use flag
        for(j=0;j<8;j++) 
        { 
        SOsdHwBuff_win[i*8+j]=0;
        }
    }
}
/////////////////////////////////////////////////
...
void SOsdHwBuffWrite2Hw(void)
{
  ...
  McuSpiClkToPclk(0x00);
	...
	McuSpiClkRestore();/*modify by gao sun chun*/
	WriteTW88Page( PAGE4_SOSD );
...
void SOsdHwBuffWrite_Win(unsigned char winno,unsigned char WaitFlag)
{
	...
	WriteTW88Page( PAGE4_SOSD );    //afei del
	WriteTW88( 0x10, 0xa0 );    	// LUT Write Mode, En & byte ptr inc. DMA needs it.
	WriteTW88( 0x11, 0); 					// LUT addr. set 0 on DMA
...
/******************************************************************************************
函数名：void disPicture(UINT8 window,const PIC_INFO_STRUCT * pPicInfo,UINT16 x,UINT16 y)
功能：在window指定的区域内显示一张图片
参数说明：UINT8 window -> 图片放在哪个窗口
          const PIC_INFO_STRUCT * pPicInfo -> 欲显示的图片信息
          UINT16 x -> 图片显示在LCD屏的起始坐标x.
          UINT16 y -> 图片显示在LCD屏的起始坐标y.
******************************************************************************************/
void SetPictureInfo(UINT8 window,const PIC_INFO_STRUCT *pPicInfo,UINT16 dis_x,UINT16 dis_y,UINT8 setFlag)
{
    SpiOsdWinImageLoc(window, pPicInfo->picAddr);  //设置图片在 外部Flash的起始地址。
    SpiOsdWinImageSizeWH(window,pPicInfo->picW, pPicInfo->picH);   //设置图片的宽度与高度
    if(setFlag!=0)  SpiOsdWinScreen(window,dis_x,dis_y, pPicInfo->picW, pPicInfo->picH); //设置窗口的起始位置及大小
    else SpiOsdWinScreen(window,pPicInfo->disX,pPicInfo->disY, pPicInfo->picW, pPicInfo->picH); //设置窗口的起始位置及大小
    SpiOsdWinPixelWidth(window,pPicInfo->bitsPixel);//设置图片像素颜色的位数
    SpiOsdWinLutOffset(window,pPicInfo->lutOffset);// //设置LUT偏移地址。
    SpiOsdWinBuffEnable(window, ON );//使能窗口。
    if(pPicInfo->rle)
	{
		SOsdHwBuffSetRle(window,pPicInfo->bitsPixel,pPicInfo->rle);
	}
	SOsdHwBuffSetLut(window, pPicInfo->lutOffset, pPicInfo->lutSize,pPicInfo->lutAddr);  //设置于LUT相关的参数
}
