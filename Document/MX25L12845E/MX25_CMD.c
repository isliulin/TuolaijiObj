 ...
 *
 * SPI and PARELLEL interface command set
 *
 * $Id: MX25_CMD.c,v 1.4 2010/02/23 05:30:28 benhuang Exp $
 */
 ...
 * Function:       IsFlashPAR
 * Arguments:      fsptr, pointer of flash status structure
 * Description:    Return flash current state: SPI or PARALLEL
...
ReturnMsg CMD_RDID( uint32 *Identification, FlashStatus *fsptr )
{
    ...
    // Check current state: SPI or PARELLEL
    if( IsFlashPAR( fsptr ) )
        io_mode = PIO;
    else
        io_mode = SIO;
 ...
 * Arguments:      REMS_Identification, 16 bit buffer to store id
 *                 fsptr, pointer of flash status structure
 * Description:    The REMS4D instruction is to read the Device
 *                 manufacturer ID and electric ID of 1-byte.
 * Return Message: FlashOperationSuccess
 */
...
ReturnMsg CMD_FASTDTRD( uint32 flash_address, uint8 *target_address, uint32 byte_length )
{
    ...
    InsertDummyCycle ( 6 );                         // Wait dummy cycle
...
ReturnMsg CMD_4DTRD( uint32 flash_address, uint8 *target_address, uint32 byte_length, FlashStatus *fsptr )
{
    ...
    // Write 4-I/O Read Array command
    SendByte( FLASH_CMD_4DTRD, SIO );
    SendFlashAddr( flash_address, DTQIO, addr_4byte_mode );
    InsertDummyCycle ( 8 );            // Wait 6 dummy cycle
 ...
 *                 fsptr, pointer of flash status structure
 * Description:    The CP instruction is for multiple
 *                 byte program to Flash.
...
ReturnMsg CMD_CP( uint32 flash_address, uint8 *source_address, uint32 byte_length, FlashStatus *fsptr )
{
    ...
    // Set a loop to down load whole page data into flash's buffer
    for( index=2; index < byte_length-1; index = index + 2 )
    {
        CS_High();
            ...
            if( (fsptr->ModeReg&0x80) == 0x80 ){
                ...
                if( !WaitFlashReady( ByteProgramCycleTime, fsptr ) )
                    return FlashTimeOut;
            }
        ...
        // Address increase automatically
 ...
 * Function:       CMD_DP
 * Arguments:      None.
 * Description:    The DP (Deep Power-down)instruction is for setting the
 ...
 * Function:       CMD_RDP
 * Arguments:      None.
 * Description:    The RDP (Release from Deep Power-down) instruction is
 ...
 * Function:       CMD_ENPLM
 * Arguments:      fsptr, pointer of flash status structure
 * Description:    The ENPLM instruction is for entering parallel mode.
 * Return Message: FlashOperationSuccess
 */
ReturnMsg CMD_ENPLM( FlashStatus *fsptr )
{
    ...
    // Write command
    SendByte( FLASH_CMD_ENPLM, SIO );

    // Chip select go high to end a flash command
    CS_High();

    fsptr->ModeReg = ( fsptr->ModeReg|0x02 );
 ...
 * Function:       CMD_EXPLM
 * Arguments:      fsptr, pointer of flash status structure
 * Description:    The EXPLM instruction is for exiting parallel mode.
 * Return Message: FlashOperationSuccess
 */
ReturnMsg CMD_EXPLM( FlashStatus *fsptr )
{
    ...
    // Write command
    SendByte( FLASH_CMD_EXPLM, SIO );

    // Chip select go high to end a flash command
    CS_High();

    fsptr->ModeReg = ( fsptr->ModeReg&0xfd );
